...
def make_file(filename, length, temperature, left_a_num, left_b_num, right_a_num, right_b_num):
    with open(filename, "w") as f:
        ...
        f.write("d(x) = (dl+dg)/2 + ((dl-dg)/2)*tanh(x-a/2*b)'\n")
        f.write("fit d(x) 'azeotrope/yz/L80T1.0.dat' u 1:2 via a, b, dl, dg")
        f.write("plot density/density/L{}T{}/lan{}-lbn{}-ran{}-rbn{}.density u 1:2 title 'density' with points pt 1, d(x) title 'density-fit' with line lt 1 lc rgb hsv2rgb(0, 1, 1)".format(length, temperature, left_a_num, left_b_num, right_a_num, right_b_num))
