...
def make_file(filename, length, temperature, left_a_num, left_b_num, right_a_num, right_b_num):
    with open(filename, "w") as f:
        ...
        f.write("fit da(x) 'density/density/L{}T{}/lan{}-lbn{}-ran{}-rbn{}.density' u 1:2 via a, b, dal, dag".format(length, temperature, left_a_num, left_b_num, right_a_num, right_b_num))
        f.write("fit db(x) 'density/density/L{}T{}/lan{}-lbn{}-ran{}-rbn{}.density' u 1:3 via c, d, dbl, dbg".format(length, temperature, left_a_num, left_b_num, right_a_num, right_b_num))
        f.write("plot density/density/L{}T{}/lan{}-lbn{}-ran{}-rbn{}.density u 1:2 title 'density_a' with points pt 1, da(x) title 'density_a-fit' with line lt 1 lc rgb hsv2rgb(0, 1, 1)".format(length, temperature, left_a_num, left_b_num, right_a_num, right_b_num))
        f.write("replot density/density/L{}T{}/lan{}-lbn{}-ran{}-rbn{}.density u 1:3 title 'density_b' with points pt 1, db(x) title 'density_b-fit' with line lt 1 lc rgb hsv2rgb(1, 1, 0)".format(length, temperature, left_a_num, left_b_num, right_a_num, right_b_num))
