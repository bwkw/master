# パラメータファイルを作成
# 系の長さ（Length）、Aの組成比（CompositionRatio）、温度（Temperature）、イプシロン（variable_epsilon)
...
for i in range(1, composition_number):
    ...
    make_file("data/param/L{}T{}E{}/AC{}.param".format(length, temperature, variable_epsilon, a_composition_ratio), length, a_composition_ratio, temperature, variable_epsilon)
