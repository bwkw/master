...
def make_file(filename, left_num_a, left_num_b, left_density_a, left_density_b, temperature):
    with open(filename, "w") as f:
        ...
        f.write("plot 'density_dat/T{4}/lna{0}-lnb{1}-lda{2}-ldb{3}.dat' u 1:2 title '粒子A', 'density_dat/T{4}/lna{0}-lnb{1}-lda{2}-ldb{3}.dat' u 1:3 title '粒子B',\n".format(left_num_a, left_num_b, left_density_a, left_density_b, temperature))
...
for i in range(1, int(sys.argv[1])):
    ...
    if not os.path.exists('density_make_plt/T{}'.format(temperature)):
        os.mkdir(('density_make_plt/T{}'.format(temperature)))
    if not os.path.exists('density_img/T{}'.format(temperature)):
        os.mkdir(('density_img/T{}'.format(temperature)))

    make_file("density_make_plt/T{}/lna{}-lnb{}-lda{}-ldb{}.plt".format(temperature, left_num_a, left_num_b, left_density_a, left_density_b), left_num_a, left_num_b, left_density_a, left_density_b, temperature)
