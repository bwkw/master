# パラメータファイルを作成
# 系の長さ（Length）、Aの組成比（CompositionRatio）、温度（Temperature）、シグマ（variable_sigma)
...
for i in range(1, composition_number):
    ...
    make_file("data/param/L{}T{}S{}/AC{}.param".format(length, temperature, variable_sigma, a_composition_ratio), length, a_composition_ratio, temperature, variable_sigma)
