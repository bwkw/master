...
def make_file(filename, length, temperature, left_a_num, left_b_num, right_a_num, right_b_num):
    with open(filename, "a") as f:
        f.write("srun --exclusive --mem-per-cpu=1840 -n 128 -c 1 -N 1 lammps < data/in.melt/L{}T{}/lan{}-lbn{}-ran{}-rbn{}.in &\n"
...
make_base_file("task/lammps/run/L{}T{}C{}.sh".format(length, temperature, composition_number))
...
for i in range(1, composition_number):
    ...
    make_file("task/lammps/run/L{}T{}C{}.sh".format(length, temperature, composition_number), length, temperature, left_a_num, left_b_num, right_a_num, right_b_num)

make_end_file("task/lammps/run/L{}T{}C{}.sh".format(length, temperature, composition_number))
