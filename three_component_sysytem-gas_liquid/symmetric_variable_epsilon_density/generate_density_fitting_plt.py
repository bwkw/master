...
def make_file(filename, length, temperature, variable_epsilon, c_density, left_a_num, left_b_num, left_c_num, right_a_num, right_b_num, right_c_num):
    with open(filename, "w") as f:
        ...
        f.write("plot 'density/density/L{0}T{1}E{2}CD{3}/lan{4}-lbn{5}-lcn{6}-ran{7}-rbn{8}-rcn{9}.density' u 1:2 title 'density_a' with points pt 1, 'density/density/L{0}T{1}E{2}CD{3}/lan{4}-lbn{5}-lcn{6}-ran{7}-rbn{8}-rcn{9}.density' u 1:3 title 'density_b' with points pt 1, 'density/density/L{0}T{1}E{2}CD{3}/lan{4}-lbn{5}-lcn{6}-ran{7}-rbn{8}-rcn{9}.density' u 1:4 title 'density_c' with points pt 1, da(x) title 'density_a-fit' with line lt 1 lc rgb hsv2rgb(0, 1, 1), db(x) title 'density_b-fit' with line lt 1 lc rgb hsv2rgb(1, 1, 0), dc(x) title 'density_c-fit' with line lt 1 lc rgb hsv2rgb(1, 0, 1)".format(length, temperature, variable_epsilon, c_density, left_a_num, left_b_num, left_c_num, right_a_num, right_b_num, right_c_num))
...
if not os.path.exists('density/density_fitting_img'):
    os.mkdir('density/density_fitting_img')
...
if not os.path.exists('density/density_fitting_img/L{}T{}E{}CD{}'.format(length, temperature, variable_epsilon, c_density)):
    os.mkdir('density/density_fitting_img/L{}T{}E{}CD{}'.format(length, temperature, variable_epsilon, c_density))
